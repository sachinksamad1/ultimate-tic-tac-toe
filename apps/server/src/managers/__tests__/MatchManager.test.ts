...
describe('MatchManager', () => {
  ...
  it('allows players to join a match', () => {
    ...
    expect(manager.joinMatch(state.matchId, player1)).toBe(true);
    expect(manager.joinMatch(state.matchId, player2)).toBe(true);
  ...
  it('rejects joining if match is full', () => {
    ...
    expect(manager.joinMatch(state.matchId, { id: '3', username: 'c', symbol: 'X' })).toBe(false);
  });
});
