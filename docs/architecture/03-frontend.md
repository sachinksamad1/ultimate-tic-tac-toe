...
3.  **Local Actions**: When a user clicks a `Cell`, it triggers a function that validates the move against the *current local state* and emits a `make_move` event to the backend.
